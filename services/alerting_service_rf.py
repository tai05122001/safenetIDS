#!/usr/bin/env python3
"""
Safenet IDS - Alerting Service (Random Forest)
Đọc từ topic level_2_predictions_rf và level_3_predictions_rf, tạo alerts và gửi đến topic alert_rf
"""
...
class AlertingService:
    ...
    def __init__(self,
                 ...
                 output_topic='alert_rf',
                 db_path='services/data/alerts_rf.db',
    ...
    def _init_database(self):
        ...
        try:
            ...
            # Tạo bảng alerts_rf (riêng biệt cho RF pipeline)
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS alerts_rf (
            ...
            # Tạo bảng alert_stats_rf (riêng biệt cho RF pipeline)
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS alert_stats_rf (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    timestamp TEXT,
                    attack_type TEXT,
                    count INTEGER,
            ...
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='alerts_rf'")
            table_exists = cursor.fetchone()
            if table_exists:
                logger.info(f"Database initialized successfully at {self.db_path.absolute()}")
                logger.info("Tables created: alerts_rf, alert_stats_rf")
            else:
    ...
    def _init_consumer(self):
        """Khởi tạo Kafka consumer - đọc từ cả level_2_predictions_rf và level_3_predictions_rf"""
        try:
            # Đọc từ cả 2 topics: level_2_predictions_rf (ddos, portscan) và level_3_predictions_rf (dos chi tiết)
            topics = ['level_2_predictions_rf', 'level_3_predictions_rf']
            self.consumer = KafkaConsumer(
                *topics,
                bootstrap_servers=self.kafka_servers,
                group_id='safenet-rf-alerting-group',
    ...
    def _should_create_alert(self, prediction: Dict[str, Any]) -> bool:
        ...
        """
        # Handle different prediction formats from RF and CNN services
        # RF services use 'label' field, CNN services use 'predicted_attack_type' or 'predicted_dos_variant'
        if 'label' in prediction:
            # RF service: label is string (dos, ddos, portscan, etc.)
        ...
        elif 'encoded' in prediction:
            # RF service: có encoded nhưng không có label, map từ encoded
            encoded_val = prediction.get('encoded', -1)
            attack_type_map = {0: 'dos', 1: 'ddos', 2: 'portscan'}
            attack_type = attack_type_map.get(encoded_val, 'unknown')
        else:
    ...
    def _generate_alert_description(self, prediction_result: Dict[str, Any]) -> str:
        ...
        if level3_pred:
            # Level 3: DoS chi tiết - RF service trả về 'label', CNN service trả về 'predicted_dos_variant'
            attack_type = level3_pred.get('label', level3_pred.get('predicted_dos_variant', 'Unknown'))
            confidence = level3_pred.get('confidence', 0.0)
            group = 'dos'
        else:
            # Level 2: ddos, portscan
            # RF service trả về 'label' thay vì 'predicted_attack_type'
            attack_type = level2_pred.get('label', level2_pred.get('predicted_attack_type', 'Unknown'))
            confidence = level2_pred.get('confidence', 0.0)
            group = level2_pred.get('label', level2_pred.get('predicted_attack_type', 'unknown'))
    ...
    def _create_alert(self, prediction_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if level3_pred:
            # RF service trả về 'label', CNN service trả về 'predicted_dos_variant'
            attack_type_raw = level3_pred.get('label', level3_pred.get('predicted_dos_variant', 'Unknown'))
            confidence = level3_pred.get('confidence', 0.0)
            prediction_level = 'Level 3 (DoS Detail)'
        else:
            # RF service trả về 'label' thay vì 'predicted_attack_type'
            attack_type_raw = level2_pred.get('label', level2_pred.get('predicted_attack_type', 'Unknown'))
            confidence = level2_pred.get('confidence', 0.0)
    ...
    def _save_alert_to_db(self, alert: Dict[str, Any]):
        ...
        """
        try:
            logger.debug(f"Attempting to save alert to database: {alert.get('alert_id')}, attack_type={alert.get('attack_type')}")
            ...
            if not table_exists:
                logger.error("Alerts_rf table does not exist, recreating database...")
                self._init_database()
                cursor = self.db_conn.cursor()
            ...
            logger.debug(f"Inserting alert: alert_id={alert.get('alert_id')}, attack_type={alert.get('attack_type')}, "
                        f"severity={alert.get('severity')}, confidence={alert.get('confidence')}")
            
            cursor.execute('''
                INSERT INTO alerts_rf (
            ...
            self.db_conn.commit()
            logger.debug(f"Alert committed to database: {alert['alert_id']}")
    ...
    def _update_alert_stats(self):
        ...
        try:
            ...
            for attack_type, count in self.alert_stats.items():
                cursor.execute('''
                    INSERT INTO alert_stats_rf (timestamp, attack_type, count, time_window)
                    VALUES (?, ?, ?, ?)
    ...
    def process_prediction_result(self, prediction_result: Dict[str, Any], original_key: str = None, topic_name: str = None):
        ...
        try:
            ...
            if 'level3_prediction' in prediction_result:
                ...
                logger.info(f"Processing Level 3 prediction from topic '{topic_name}': "
                          f"attack_type={attack_type}, confidence={confidence:.2f}")

                # Debug: Log chi tiết về _should_create_alert
                label_from_pred = level3_pred.get('label', 'N/A')
                confidence_from_pred = level3_pred.get('confidence', 0.0)
                attack_type_lower = attack_type.lower() if isinstance(attack_type, str) else str(attack_type).lower()
                threshold = self.alert_thresholds.get(attack_type_lower, self.alert_thresholds['default'])
                logger.info(f"Level 3 alert check - label='{label_from_pred}', attack_type_lower='{attack_type_lower}', "
                          f"confidence={confidence_from_pred:.2f}, threshold={threshold:.2f}, "
                          f"should_create={confidence_from_pred >= threshold}")

                if self._should_create_alert(level3_pred):
                    # Tạo alert từ Level 3 (DoS chi tiết)
                    logger.info(f"Creating Level 3 alert for attack_type={attack_type}")
                    alert = self._create_alert(prediction_result)
                    logger.info(f"Alert object created: alert_id={alert.get('alert_id')}, attack_type={alert.get('attack_type')}")

                    # Lưu vào database
                    try:
                        self._save_alert_to_db(alert)
                        logger.info(f"Level 3 alert successfully saved to database: {alert['alert_id']}")
                    except Exception as db_error:
                        logger.error(f"Failed to save Level 3 alert to database: {db_error}")
                        import traceback
                        logger.error(traceback.format_exc())
                    ...
                    try:
                        ...
                        logger.info(f"Level 3 alert successfully sent to Kafka: {alert['alert_id']}")
                    except Exception as kafka_error:
                        logger.error(f"Failed to send Level 3 alert to Kafka: {kafka_error}")
                    ...
                    confidence = level3_pred.get('confidence', 0.0)
                    logger.info(f"Alert not created for DoS (Level 3) - confidence too low: {confidence:.2f} < threshold: {threshold:.2f}")
            ...
            # Kiểm tra có phải kết quả Level 2 hay không (ddos, portscan)
                ...
                # Nếu có 'encoded', có thể map từ int sang string
                if attack_type_raw == 'Unknown' and 'encoded' in level2_pred:
                    encoded_val = level2_pred.get('encoded', -1)
                    attack_type_map = {0: 'dos', 1: 'ddos', 2: 'portscan'}
                    attack_type_raw = attack_type_map.get(encoded_val, 'Unknown')
                ...
                # Nếu từ level_2_predictions_rf và là dos -> đợi Level 3
                if topic_name == 'level_2_predictions_rf' and attack_type == 'dos':
                    logger.debug(f"DoS attack detected at Level 2 from topic '{topic_name}' - "
                               f"waiting for Level 3 detail prediction from level_3_predictions_rf")
    ...
    def start_alerting(self):
        ...
        logger.info(f"Starting alerting service (RF): reading from level_2_predictions_rf and level_3_predictions_rf -> {self.output_topic}")
        ...
        try:
            for message in self.consumer:
                ...
                try:
                    ...
                    logger.info(f"Received message from topic '{topic_name}' for key: {original_key}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Safenet IDS - Alerting Service (Random Forest)')
    ...
    parser.add_argument('--input-topic', default='level_2_predictions_rf',
                       help='Input topic name (deprecated - service reads from both level_2_predictions_rf and level_3_predictions_rf)')
    parser.add_argument('--output-topic', default='alert_rf',
                       help='Output topic name (IDS alerts)')
    parser.add_argument('--db-path', default='services/data/alerts_rf.db',
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('services/logs/alerting_rf.log'),
    ...
    # Tạo logger
    global logger
    logger = logging.getLogger('AlertingRF')
    ...
    try:
        logger.info("Starting Safenet IDS Alerting Service (Random Forest)...")
