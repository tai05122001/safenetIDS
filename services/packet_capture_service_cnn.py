#!/usr/bin/env python3
"""
Safenet IDS - Real-time Packet Capture Service (CNN Pipeline)
...
- Gửi đến Kafka topic raw_data_event_cnn (CNN Pipeline)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('services/logs/packet_capture_cnn.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('PacketCaptureCNN')
...
class RealTimePacketCapture:
    ...
    def _send_to_kafka(self, features):
        ...
        try:
            # Thêm id field để dễ tracking (cho CNN preprocessing service)
            if 'id' not in features:
                flow_id = f"cnn_flow_{int(time.time() * 1000)}_{self.total_flows}"
                features['id'] = flow_id
            
            # Đảm bảo có Label field (CNN preprocessing service tìm 'Label', 'label', etc.)
...
def list_interfaces():
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("Usage: python packet_capture_service_cnn.py --interface <interface_name>")
        logger.info("Example: python services/packet_capture_service_cnn.py --interface \"Ethernet\"")
    except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Safenet IDS - Real-time Packet Capture Service (CNN Pipeline) (tshark/pyshark/CICFlowMeter)',
...
Examples:
  ...
  python services/packet_capture_service_cnn.py
  
  # Real-time capture với interface cụ thể (Windows)
  python services/packet_capture_service_cnn.py --interface "Ethernet"
  
  # Real-time capture với interface cụ thể (Linux)
  python services/packet_capture_service_cnn.py --interface eth0
  
  # List available interfaces
  python services/packet_capture_service_cnn.py --list-interfaces
  
  # [TÙY CHỌN] Process PCAP file offline (auto-detect CICFlowMeter or use pyshark)
  python services/packet_capture_service_cnn.py --pcap-file data/traffic.pcap
  
  # Process PCAP file with CICFlowMeter (force) - đủ 79 features
  python services/packet_capture_service_cnn.py --pcap-file data/traffic.pcap --use-cicflowmeter
  
  # Process PCAP file with pyshark only
  python services/packet_capture_service_cnn.py --pcap-file data/traffic.pcap --no-cicflowmeter
  
  # Custom Kafka settings
  python services/packet_capture_service_cnn.py --interface eth0 --kafka-servers localhost:9092 --topic raw_data_event_cnn
        """
    )
    
    parser.add_argument('--kafka-servers', default='localhost:9092',
                       help='Kafka bootstrap servers (default: localhost:9092)')
    parser.add_argument('--topic', default='raw_data_event_cnn',
                       help='Kafka topic name (default: raw_data_event_cnn)')
