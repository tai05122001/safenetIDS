...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Huấn luyện mô hình IDS level 1 với Random Forest."
    )
    parser.add_argument(
        "--splits-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Danh sách cột bỏ qua khi huấn luyện (ví dụ cột label_group, label gốc, label_encoded, label_group_encoded, label_attack_type_encoded).",
    )
    parser.add_argument(
        "--test-size",
        type=float,
...
def build_model_pipeline(
    ...
    Sử dụng custom class_weight để giảm bias về benign và tăng trọng số cho attack.
    """
    # Custom class weights cho binary classification
    # 0: benign, 1: attack
    class_weights = {
        0: 0.5,   # Giảm weight cho benign (vì có quá nhiều trong dataset gốc)
        1: 2.0,   # Tăng weight cho attack (quan trọng hơn)
    }
    ...
    if y_train is not None:
        ...
        # Điều chỉnh: Tăng weight cho attack class và giảm cho benign
        for cls in classes:
            if cls == 0:  # benign
                computed_dict[cls] = computed_dict[cls] * 0.5  # Giảm 50%
            elif cls == 1:  # attack
                computed_dict[cls] = computed_dict[cls] * 2.0  # Tăng 100%
        
        # Merge với default weights (cho các class không có trong y_train)
        for cls, weight in class_weights.items():
            if cls not in computed_dict:
                computed_dict[cls] = weight
        
        class_weights = computed_dict
        logging.info(f"Computed class weights for binary classification: {class_weights}")
    else:
    ...
    classifier = RandomForestClassifier(
        ...
        n_jobs=-1,
        random_state=42,
        class_weight=class_weights,  # Sử dụng custom weights thay vì "balanced_subsample"
    )
...
def save_artifacts(
    ...
    model_path = output_dir / "ids_pipeline_rf.joblib"
...
def main() -> None:
    ...
    run_training_pipeline(
        ...
        min_samples_leaf=args.min_samples_leaf if hasattr(args, 'min_samples_leaf') else 1,
    )
...
def run_training_pipeline(
    *,
    ...
    output_dir: Path | str = Path("artifacts_rf"),
    n_estimators: int = 300,
    max_depth: int | None = None,
    min_samples_split: int = 2,
    min_samples_leaf: int = 1,
) -> Dict[str, object]:
    """Chạy toàn bộ quy trình huấn luyện level 1 với Random Forest."""
    setup_logging()
    ...
    if train_file.exists():
        # Kiểm tra nhanh số dòng trong file
        try:
            import pickle
            with open(train_file, 'rb') as f:
                df_check = pickle.load(f)
                if isinstance(df_check, pd.DataFrame):
                    logging.info(f"📁 File có {df_check.shape[0]} rows, {df_check.shape[1]} cols")
                    if label_column in df_check.columns:
                        label_counts_check = df_check[label_column].value_counts().sort_index()
                        logging.info(f"📁 Class distribution trong file:")
                        for lbl, cnt in label_counts_check.items():
                            logging.info(f"     {lbl}: {cnt}")
        except Exception as e:
            logging.warning(f"Không thể kiểm tra nội dung file: {e}")
    logging.info("=" * 80)
    ...
    if label_column in df_train.columns:
        ...
        # ⚠️ CẢNH BÁO: Nếu đang dùng train_variant="balanced" nhưng benign vẫn > 660000
        # → Có thể đang load nhầm train_raw.pkl thay vì train_balanced.pkl
        if train_variant == "balanced":
            benign_labels = [l for l in train_label_counts.index if l == 0 or (isinstance(l, str) and 'benign' in str(l).lower())]
            if benign_labels:
                benign_count = sum(train_label_counts[l] for l in benign_labels if l in train_label_counts.index)
                if benign_count > 210000:
                    logging.error("=" * 80)
                    logging.error("⚠️  CẢNH BÁO NGHIÊM TRỌNG!")
                    logging.error("⚠️  Đang dùng train_variant='balanced' nhưng benign có %d samples (>210000)", benign_count)
                    logging.error("⚠️  Có thể đang load nhầm train_raw.pkl thay vì train_balanced.pkl!")
                    logging.error("⚠️  Kiểm tra lại file: %s", train_file)
                    logging.error("=" * 80)
        
        # Binary classification: 0=benign, 1=attack
        # Kiểm tra xem có đúng 2 classes không
        unique_classes = sorted(train_label_counts.index)
        if len(unique_classes) != 2 or 0 not in unique_classes or 1 not in unique_classes:
            logging.warning(
                "⚠️  Binary classification cần 2 classes (0=benign, 1=attack). "
                "Phát hiện classes: %s", unique_classes
            )
        
        # Kiểm tra xem benign (class 0) đã được giảm xuống train_max chưa
        if 0 in train_label_counts.index:
            benign_count = train_label_counts[0]
            if benign_count > 660000:  # Nếu vẫn còn > 660K thì có thể chưa được balance
                logging.warning(
                    "Benign (class 0) có %d samples - có thể chưa được giảm xuống train_max (650000). "
                    "Kiểm tra lại split_dataset.py có áp dụng train_max=650000 không.",
                    benign_count
                )
            else:
                logging.info(
                    "✓ Benign (class 0) đã được giảm xuống %d samples (target: ≤650000, tỷ lệ ~1.2:1 với attack)",
                    benign_count
                )
        
        # Kiểm tra attack (class 1)
        if 1 in train_label_counts.index:
            attack_count = train_label_counts[1]
            logging.info(
                "✓ Attack (class 1) có %d samples",
                attack_count
                )
    ...
    logging.info(
        "Bắt đầu huấn luyện Random Forest (train=%d)...",
        X_train.shape[0]
    )
    ...
    metadata = {
        ...
        "level": 1,
        "level_description": "Binary Classification (benign vs attack)",
        "n_estimators": n_estimators,
        "max_depth": max_depth,
        "min_samples_split": min_samples_split,
        "min_samples_leaf": min_samples_leaf,
